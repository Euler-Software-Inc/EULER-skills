...
| [`my-performance`](./skills/my-performance/SKILL.md) | 🆕 New *(field paths pending live-MCP validation)* | Partner-facing: a partner views their own performance scorecard with one customer — health score (0–100), headline stats (revenue, deals, commissions, referrals), and the single biggest lever to improve (contrast: `generate-qbr` is the customer admin reviewing a partner; this is that partner reviewing themselves) |
| [`my-referrals`](./skills/my-referrals/SKILL.md) | 🆕 New *(field paths pending live-MCP validation)* | Partner-facing: a partner views their full referral history with one customer — pipeline snapshot (active, won, lost counts + total pipeline value), per-referral status cards sorted by recency, and next-action nudges for stale or stuck referrals |
| [`my-deals`](./skills/my-deals/SKILL.md) | 🆕 New *(field paths pending live-MCP validation)* | Partner-facing: a partner views all deal registrations they have submitted with one customer — status breakdown (approved, pending, rejected), per-deal cards with value and stage, and a summary of total attributed pipeline |
| [`submit-a-referral`](./skills/submit-a-referral/SKILL.md) | 🆕 New *(field paths pending live-MCP validation)* — **write action** | Partner-facing: a partner submits a new referral to a customer — collects company name, contact, and opportunity details interactively, then calls the EULER MCP to create the referral record |
| [`my-tracking-links`](./skills/my-tracking-links/SKILL.md) | 🆕 New *(field paths pending live-MCP validation)* — **write action** | Partner-facing: a partner views their attribution tracking links and generates new ones — lists all existing links with click stats, and creates a new link for a specified campaign or page on request |
