...
- `/euler-for-partners:my-referrals`
...
3. **01 · Your referrals** — `.section-eyebrow` "01 · Your referrals"; `.table-wrap` table:
   **Company · Type · Status · Submitted** (columns). Most-recent first. Status as `.status-pill`:
   ...
   "No referrals yet — submit your first with `/euler-for-partners:submit-a-referral`." (localized).
   After the table: a `.note` CTA — "To submit a new referral, use `/euler-for-partners:submit-a-referral`."
...
8. **Read-only** — this skill shows referral status; it **never** submits a referral, changes a
   status, or claims to have done so. Direct submission to `/euler-for-partners:submit-a-referral`.
